...
\newcommand{\met}{\textsf{acopost-met}\xspace}
\newcommand{\ttt}{\textsf{acopost-t3}\xspace}
\newcommand{\tbt}{\textsf{acopost-tbt}\xspace}
\newcommand{\et}{\textsf{acopost-et}\xspace}
...
\begin{center}
  ...
  \normalsize\normalfont
  ACOPOST Developers Team, \\
  Ingo Schr\"{o}der and others
\end{center}
...
\acopost is available under the BSD 3 clauses license
...
No pre-compiiled binaries are available but the typical
...
procedure should work in most Unix-like environments.
You only need a modern C compiler (gcc is recommended),
...
\begin{quote}
\begin{verbatim}
PROMPT> gunzip -c acopost-2.0.0.tar.gz | tar fxv -
acopost-2.0.0/
acopost-2.0.0/src/
acopost-2.0.0/src/Makefile
acopost-2.0.0/src/array.c
...
\end{verbatim}
\end{quote}
...
To configure, type
\verb+./configure+. If everything works out ok, this command will create
a Makefile in the current directory. By default \verb+./configure+ will create
a Makefile that will install everything in \verb+/usr/local+.
You can change this behaviour by specifing a different destination path using \verb+--prefix+ option:
\verb+./configure --prefix=<path>+.
To compile, type
\verb+make+. If everything works out ok, issue the command 
\verb+make install+ which installs the binaries into the directory 
\verb+<path>/bin+. Congratulations! You're done. 

If something goes wrong, try to fix it by configure options
or the source code. Don't forget to tell us about your problems so
that we can provide a better solution with the next release.
...
\verb+<path>/bin+ directory to a directory already in your \verb+PATH+
...
File formats have been designed to allow one
to use standard text editing tools on the corpora, \eg UNIX tools like 
\verb+grep+, \verb+sed+, \verb+wc+ etc.\ or Perl. Therefore,
line-based formats are used for the corpora, \ie each line of texts (separated
...
\item
  ...
  more white space characters, \ie in the same way adjacent words are
...
\verb+acopost-wsj2cooked+ (cf.\ Section~\ref{S:wsj2cooked}),
\verb+acopost-tt2cooked+ (cf.\ Section~\ref{S:tt2cooked}) and
\verb+acopost-cooked2tt+ (cf.\ Section~\ref{S:cooked2tt}).
...
Note that not all programs in \acopost are described
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
\subsection{acopost-complementary-rate}
\label{S:complementary-rate}
...
\verb+acopost-complementary-rate [OPTIONS] referencefile a b+

where 
\verb+referencefile+ is a reference corpus in cooked format
and
\verb+a+ and \verb+b+ are tagged corpora in cooked format.
\verb+OPTIONS+ can be:
...
\begin{verbatim}
PROMPT> acopost-complementary-rate 0.ref 0.t3 0.tnt
...
\verb+acopost-cooked2lex [OPTIONS] < in.cooked > out.lex+
...
\begin{verbatim}
PROMPT> acopost-cooked2lex < negra.cooked > negra.lex
20602 sentences
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
\subsection{acopost-cooked2ngram}
\label{S:cooked2ngram}
...
\verb+acopost-cooked2ngram [OPTIONS] < in.cooked > out.ngram+
...
\begin{verbatim}
PROMPT> acopost-cooked2ngram < corpus.cooked > corpus.ngram
\end{verbatim}

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
\subsection{acopost-cooked2tt}
\label{S:cooked2tt}
...
\verb+acopost-cooked2tt [OPTIONS] < in.cooked > out.tt+
...
\begin{verbatim}
PROMPT> acopost-cooked2tt < negra.cooked > negra.tt
20602 sentences
PROMPT>
\end{verbatim}
...
\verb+acopost-cooked2wsj [OPTIONS] < in.cooked > out.wsj+
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
\subsection{acopost-cooked2wtree}
\label{S:cooked2wtree}
...
\verb+acopost-cooked2wtree [OPTIONS] f-file < in.cooked > out.wtree+
...
\begin{tabular}{lp{15cm}}
\verb+-h+ & display a short help text and exit \\
\verb+-o mode+ &  any of ``'' or \verb+debug+, changing the behaviour of the command (default: ``''). \\
\verb+-A a+ & 
...
\verb+acopost-cooked2wtree+ 
(cf.\ Section~\ref{S:cooked2wtree}).

\subsubsection{Usage}

\verb+acopost-et [OPTIONS] knownwtree unknownwtree [inputfile] > out.cooked+
...
\verb+-l lexiconfile+ &  a lexicon file generated by
\verb+acopost-cooked2lex+
(cf.\ Section~\ref{S:cooked2lex}). \\
\verb+-o mode+ &  any of \verb+tag+ or \verb+test+, changing the behaviour of the command (default: tag). \\
\end{tabular}
...
\begin{small}
\begin{verbatim}
PROMPT> acopost-cooked2lex < train.cooked > train.lex
...
PROMPT> acopost-cooked2wtree -A 3 known.etf < train.cooked > known.wtree
...
PROMPT> acopost-cooked2wtree -B 2 unknown.etf -X closed-class-tags < train.cooked > unknown.wtree
...
PROMPT> acopost-et -l train.lex known.wtree unknown.wtree < test.raw > test.et
[        0 ms::1]
...
[     3590 ms::1] done
PROMPT> acopost-evaluate test.cooked test.et
2060 sentences
...
\verb+acopost-evaluate [OPTIONS] referencefile [testfile ...]+

where 
\verb+referencefile+ is a reference corpus in cooked format
and
\verb+testfile+ is a tagged corpus in cooked format.
\verb+OPTIONS+ can be:
...
(cf.\ Section~\ref{S:cooked2lex}). \\
\verb+-C+ &
case-sensitive mode for lexicon
\end{tabular}

\subsubsection{Example}

\begin{verbatim}
PROMPT> acopost-evaluate 0.ref 0.t3 0.tnt
1002 sentences
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
\subsection{acopost-majority-voter}
\label{S:majority-voter}
...
\verb+acopost-majority-voter [OPTIONS] referencefile [taggedfile ...]+

where 
\verb+referencefile+ is a reference corpus in cooked format
and
\verb+taggedfile+ is a tagged corpus in cooked format.
\verb+OPTIONS+ can be:
...
\begin{verbatim}
PROMPT> acopost-majority-voter 0.ref 0.t3 0.tbt 0.et 0.met
2061 sentences
35674 words
...
% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
\subsection{acopost-met}
\label{S:met}

\subsubsection{Purpose}

Assign tags to a natural language text in raw format using the
maximum entropy paradigm.

\subsubsection{Usage}

\verb+acopost-met [OPTIONS] modelfile [inputfile]+
...
\verb+-C+ &
case-sensitive mode for lexicon \\
\verb+-o mode+ &  any of \verb+tag+, \verb+train+ or \verb+test+, changing the behaviour of the command (default: tag).\\
\verb+-b beamfactor+ &
...
\begin{small}
\begin{verbatim}
PROMPT> acopost-met -o test -l train.lex train.model.met < test.cooked
...
\verb+acopost-cooked2ngramn+ 
(cf.\ Section~\ref{S:cooked2ngram}).

\subsubsection{Usage}

\verb+acopost-t3 [OPTIONS] modelfile [in.raw] > out.cooked+

where \verb+modelfile+ is a tag trigram file generated by 
\verb+acopost-cooked2ngram+
(cf.\ Section~\ref{S:cooked2ngram}).
If the input file
\verb+in.raw+
is omitted standard input is used. 
...
\verb+acopost-cooked2lex+
(cf.\ Section~\ref{S:cooked2lex}). \\
\verb+-o mode+ &  any of \verb+tag+, \verb+test+, \verb+dump+ or \verb+debug+, changing the behaviour of the command (default: tag).\\
\verb+-a a+ & 
...
PROMPT> acopost-cooked2lex < train.cooked > train.lex
...
PROMPT> acopost-cooked2ngram < train.cooked > train.ngram
...
PROMPT> acopost-t3 -l train.lex train.ngram < test.raw > test.t3
[        0 ms::1]
...
[    21690 ms::1] done
PROMPT> acopost-evaluate test.cooked test.t3
2061 sentences
...
\verb+acopost-tbt [OPTIONS] rulefile [inputfile]+
...
training only \\
%
\verb+-r r+ & rare wore threshold (default: 0) \\
%
\verb+-o o+ &
...
...
PROMPT> acopost-tbt -R -l train.lex train.rules < test.raw > test.tbt
Transformation-based Tagger 
(c) Ingo Schröder, ingo@nats.informatik.uni-hamburg.de
done
PROMPT> acopost-evaluate test.cooked test.tbt
2061 sentences
...
\verb+acopost-tt2cooked [OPTIONS] < in.tt > out.cooked+
...
\begin{verbatim}
PROMPT> acopost-tt2cooked < negra.tt > negra.cooked
396309 lines read
PROMPT>
\end{verbatim}

% - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
\subsection{acopost-wsj2cooked}
\label{S:wsj2cooked}
...
\verb+acopost-wsj2cooked < in.wsj > out.cooked+

\subsubsection{Example}

\begin{verbatim}
PROMPT> acopost-wsj2cooked < corpus.wsj > negra.cooked
PROMPT>
\end{verbatim}
